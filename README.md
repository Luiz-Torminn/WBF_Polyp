...
evaluates it with a single unified `supervision` evaluator, unloads it, and finally
...
2. **Per model**, an adapter normalizes that model's output into a shared
   ...
   batches and each model is scored in memory.
...
All metrics in `summary.csv` come from one source of truth — the `supervision`
metrics: `MeanAveragePrecision` for mAP, and `Precision`/`Recall` read at a fixed
`0.5` confidence operating point (see
[Evaluation & threshold semantics](#evaluation--threshold-semantics)).
...
│   ├── metrics.py              # Unified supervision evaluator (P/R/mAP)
...
Key packages: `ensemble-boxes` (WBF), `supervision` (metrics + ground-truth
targets), `torch`/`torchvision`, `optuna` + `optuna-dashboard` +
`psycopg2-binary` (for the hyperparameter search). `pycocotools` is a pinned
transitive dependency and is no longer the metric engine.
...
The pipeline keeps **three distinct thresholds** — do not conflate them:
...
- **P/R operating point** (`PR_CONFIDENCE_THRESHOLD`, fixed `0.5`): the
  confidence at which Precision and Recall are read (see below). It only affects
  the P/R columns — never mAP, never inference or fusion.

All `summary.csv` numbers come from one evaluator (`ensemble/metrics.py`), the
`supervision` metrics:

- **mAP@50** and **mAP@[50:95]** from `MeanAveragePrecision`, computed over the
  **full, unfiltered** prediction set (the low-confidence tail is needed for the
  PR curve).
- **Precision / Recall** from `supervision`'s `Precision` / `Recall` at IoU=0.50,
  computed only over detections with **`confidence >= 0.5`** (`>=`, so a box
  exactly at 0.5 is kept). This fixed operating point is applied uniformly to the
  standalone rows and the ENSEMBLE row.

`supervision` exposes no confidence-threshold parameter of its own (confidence is
used only to sort detections), so the `0.5` operating point is enforced by
filtering the detections before they reach the P/R metrics. Reading P/R over the
full unfiltered set instead would drive precision toward zero for the DETR models
(RFDETR/DEIMv2 emit ~300 NMS-free queries per image at `conf≈0.001`).
